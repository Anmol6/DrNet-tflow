...
def train():
    ...
    with tf.name_scope('drnet') as scope:    
        for i in range(FLAGS.num_gpus):
            ...
            with tf.device('/gpu:' + str(i)), tf.name_scope('tower_'+str(i)):
                ...
                #get all frames. 
                #images to train C
                images_1 = video_batch_tower[:FLAGS.batch_size,0,:,:,:] 
                images_4 = video_batch_tower[:FLAGS.batch_size,1,:,:,:] 
                
                #images to train everything else
                images_1_=video_batch_tower[FLAGS.batch_size:,0,:,:,:]
                images_2 = video_batch_tower[FLAGS.batch_size:,1,:,:,:] 
                images_3 = video_batch_tower[FLAGS.batch_size:,2,:,:,:]
                ...
                Ec_out = model.Ec(images_1_, FLAGS.Ec_scope, reuse_Ec = reuse_flag) 
    ...
    #train_C = tf.train.AdamOptimizer(learning_rate=FLAGS.lr).minimize(FLAGS.beta*total_loss_C, var_list  = C_vars)
    ##with tf.control_dependencies([train_C]):
    #total_loss = (FLAGS.beta*total_loss_Ep)+(FLAGS.alpha*total_loss_Ec)+FLAGS.alpha_2*total_loss_D+(FLAGS.beta*total_loss_C)
    ...
    #train_all = tf.group(train_ec_ep_d, train_C)
    ...
    #try training scene discriminator first
    '''
    for i in range(1):
        print('pre-training scene discriminator and pose encoder, iter ' + str(i))    
        sess.run(train_C)
        sess.run(train_C)
        sess.run(train_Ep)
    '''
        ...
        while not coord.should_stop(): 
            ...
            if(i%FLAGS.check_output_freq==0):    
                ...
                #np.save('tmp/npys/orig2_shuffled_'+str(i)+'_'+FLAGS.run_name+'_.npy',orig2_shuffled)
                 ...
                 #a,b,c = sess.run([images_1, images_2_same_scene,out])
                 #import ipdb
                 #ipdb.set_trace()
                 #sess.run([train_C])
...
def main():
    ...
    flags.DEFINE_string('log_dir', './logs_6/', 'Directory where to write logs')
